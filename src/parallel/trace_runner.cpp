...
void print_usage() {
  std::cerr << "Usage: par_trace_runner --graph <graph.txt> --batch <batch.txt> --out <out.json> "
            << "[--num-levels N] [--group-size G] [--delta D] [--lambda L] [--num-threads T]\n";
}
...
int main(int argc, char** argv) {
  ...
  seq::SequentialBatchEngine seq_engine(std::move(graph), config, std::move(state),
                                        std::move(descriptors));

  seq::parallel::ParallelOptions popts;
  popts.num_threads = args.num_threads;
  seq::parallel::ParallelBatchEngine engine(std::move(seq_engine), popts);

  seq::BatchUpdate empty_batch;
  (void)seq::parallel::process_batch(engine, empty_batch);

  const seq::BatchUpdate batch = load_batch(args.batch_path);
  const auto t0 = std::chrono::steady_clock::now();
  const seq::BatchResult result = seq::parallel::process_batch(engine, batch);
  ...
  out << "    \"num_threads\": " << args.num_threads << "\n";
  out << "  },\n";
  out << "  \"summary\": {\n";
  ...
  out << ",\n";
  out << "  \"descriptor\": {\n";
  out << "    \"batch_number\": " << engine.sequential.descriptors.batch_number << ",\n";
  out << "    \"old_level\": ";
  write_int_array(out, engine.sequential.descriptors.old_level);
  out << "\n";
  out << "  }\n";
  out << "}\n";
